...
void montar_simplex() {
  ...
  */
  ...
  while (1) {
    ...
    for (int a = 0; a < qtd_var; a++) {
      if (funcao_objetivo[a] >= 0 && a > 0) {
        printf("+");
      }
      printf(BOLD BLUE " %.1lfX%d "
        COLOR_OFF, funcao_objetivo[a], a + 1);
    }
    printf("\n\n");
    ...
    for (int a = 0; a < qtd_res; a++) {
      ...
      for (int b = 0; b < qtd_var; b++) {
        printf(" Qual valor deseja atribuir ao"
          BOLD BLUE " X%d "
          COLOR_OFF "da restricao"
          BOLD GREEN " [%d] : "
          COLOR_OFF, b + 1, a + 1);
        scanf("%lf", & restricoes[a][b]);
      }

      printf(" Qual o resultado da restricao"
        BOLD GREEN " [%d] ? : "
        COLOR_OFF, a + 1);
      scanf("%lf", & restricoes[a][qtd_var]);

      // Exibindo como ficou a restricao
      printf("\n Portanto a restricao"
        BOLD GREEN " %d"
        COLOR_OFF " ficou: ", a + 1);
      for (int b = 0; b < qtd_var; b++) {
        if (restricoes[a][b] >= 0 && b > 0) {
          printf("+");
        }
        printf(BOLD GREEN " %.1lfX%d "
          COLOR_OFF, restricoes[a][b], b + 1);
      }
      printf("<= "
        BOLD GREEN "%.1lf"
        COLOR_OFF, restricoes[a][qtd_var]);
      printf("\n\n");
    }
